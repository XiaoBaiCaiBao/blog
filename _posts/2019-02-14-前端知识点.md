...
- [|**对象与原型**|](#对象与原型-1)
    - [2019.2.15](#2019215)
- [|**对象**|](#对象)
  - [_属性操作（4-3）_](#_属性操作4-3_)
  - [_get/set 方法（4-4）_](#_getset-方法4-4_)
  - [_属性标签（4-5）_](#_属性标签4-5_)
  - [_对象标签，对象序列化（4-6）_](#_对象标签对象序列化4-6_)
  - [_第 4 章总结_](#_第-4-章总结_)
    - [2019.2.16](#2019216)
- [|**数组**|](#数组)
  - [_创建数组（5-1）_](#_创建数组5-1_)
  - [_二维数组和稀疏数组（5-2）_](#_二维数组和稀疏数组5-2_)
  - [_数组方法（5-3 5-4）_](#_数组方法5-3-5-4_)
  - [_数组小结_](#_数组小结_)
  - [_待解决问题_](#_待解决问题_)
- [|**函数和 this**|](#函数和-this)
  - [_函数概述（6-1）_](#_函数概述6-1_)
  - [_函数声明与表达式（6-2）_](#_函数声明与表达式6-2_)
  - [_this（6-3）_](#_this6-3_)
  - [_函数属性 arguments（6-4）_](#_函数属性-arguments6-4_)
    - [2019.2.17](#2019217)
- [|**闭包和作用域**|](#闭包和作用域)
  - [_闭包（7-1）_](#_闭包7-1_)
  - [_作用域（7-2）_](#_作用域7-2_)
  - [_ES3 执行上下文（7-3）_](#_es3-执行上下文7-3_)
- [|**OOP**|](#oop)
  - [_概念与继承（8-1）_](#_概念与继承8-1_)
  - [_再谈原型链（8-2）_](#_再谈原型链8-2_)
  - [_prototype 属性（8-3）_](#_prototype-属性8-3_)
  - [_instanceof（8-4）_](#_instanceof8-4_)
  - [_实现继承的方式（8-5）_](#_实现继承的方式8-5_)
- [|**OOP（模拟重载 链式调用 模块化）**|](#oop模拟重载-链式调用-模块化)
  - [_探测器(9-2)_](#_探测器9-2_)
- [|**Unity 和 VR**|](#unity-和-vr)
    - [2019.2.19 垃圾](#2019219-垃圾)
  - [_课程简介_](#_课程简介_)
- [|**JS 基础知识**|](#js-基础知识)
  - [_变量类型和计算_](#_变量类型和计算_)
  - [_构造函数_](#_构造函数_)
...
- [|**异步和单线程**|](#异步和单线程)
  - [_什么是异步_](#_什么是异步_)
    - [2019.2.20](#2019220)
- [|**HTTP 协议类**|](#http-协议类)
  - [_HTTP 协议类_](#_http-协议类_)
- [|**异步和性能**|](#异步和性能)
  - [_序_](#_序_)
  - [_异步：现在与将来 第 1 章_](#_异步现在与将来-第-1-章_)
    - [2019.2.21 (wang)](#2019221-wang)
- [|变量类型和计算|](#变量类型和计算)
- [|**面向对象**|](#面向对象)
  - [_面向对象_](#_面向对象_)
- [|**通信类**|](#通信类)
...
- [|**ES6**|](#es6-2)
  - [**let const**](#let-const)
  - [**解构赋值**](#解构赋值)
  - [**正则扩展** 结合阮一峰的博客学习](#正则扩展-结合阮一峰的博客学习)
  - [**字符串扩展**](#字符串扩展)
  - [**数组扩展**](#数组扩展)
  - [**函数扩展**](#函数扩展)
  - [**对象扩展**](#对象扩展)
...
- [|**表达式和运算符**|](#表达式和运算符)
  ...
  - [**运算符**](#运算符)
- [|**语句**|](#语句)
  - [**block 语句 var 语句**](#block-语句-var-语句)
  - [**函数 switch 循环**](#函数-switch-循环)
  - [**严格模式**](#严格模式)
...
- **类与实例**
  - 类的声明
    ...
    - ES6 Class(ES6 待看？)
      ...
      ```
  - 生成实例
    - 如果类不需要传参，new 实例时可以不写括号
- **类与继承**

  - **如何实现继承**（继承的本质就是原型链）(再看看？)
    ...
    - 借助构造函数实现继承
      ...
      - 缺点:不能继承父类的原型链 Parent1.prototype
    ...
    - 借助原型链实现继承
      ...
      - 缺点: 1.传参。2.原型中引用类型的属性被多个实例继承后，实例修改该属性会相互影响。原因:实例化出来的对象本身没有那个**引用类型**的属性，他们的值都是向原型链上读取原型对象的属性值，所以其中一个修改就是改变了原型对象的值，会影响另一个。
    ...
    - 组合继承的优化 2
      ...
      ```
      - 通过 object 继承，为什么 constructor 会改变
      - student.prototype.\_\_proto\_\_=person.prototype 是否会出现问题，与 object.create(person.prototype)相比会出现哪些问题，constructor 为什么会发生改变
      - instanceof 的原理 为啥原型链上的都是 true

  - 继承的几种方式

## |**通信类**|

> imooc 《前端跳槽面试必备技巧》（3-14）
...
- 如何创建 Ajax
  ...
  - 代码实现(再看看？)
    - ('Microsoft.XMLHTTP')
      ![ajax1](https://img.mukewang.com/szimg/5a02f80d0001666519201080.jpg)
      ![ajax2](https://img.mukewang.com/szimg/5a02f8630001539b19201080.jpg)
- 跨域通信的几种方式
  ...
  - JSONP:
    ...
    - 实现：（再看看？）
  - Hash:
    - 原理：URL 地址中#号后面的即为 Hash.Hash 变动页面不会刷新(原理)。URL 地址中?后面的叫 Search,Search 改变会刷新页面，所以 Search 不能做跨域通信。
    - 实现:页面 A 通过 iframe 或 frame 嵌入了跨域的页面 B
    ```
    //A
    ...
    ```
  - postMessage:HTML5 新增标准。(再看看？)

    - 实现：窗口 A 向跨域窗口 B 发送信息

    ```
    //A.`Bwindow`是B的窗口引用。第一个参数是要发送的数据，推荐字符串格式。第二个参数是接收方的源，可以写源地址，也可以写'*'(不推荐不安全)。
    <iframe src="http://B.com"></iframe>
    window.onload=function(){ //保证被嵌入页面加载完成否则报错
      window.frames[0].postMessage("datachuanshu","http://B.com");
    }

    //在B中监听message事件，和click keyup等事件用法相同。false表示冒泡触发
  ...
  - WebSocket:阮一峰？
    - 实现
  - CORS:阮一峰？通信标准，可理解为支持跨域通信的 Ajax。浏览器检测到使用了 Ajax 发送跨域请求时，会在 http 头中增加 origin，来允许跨域通信。
    - 新兴 Ajax api:fetch。

## |**安全类**|

> imooc 《前端跳槽面试必备技巧》（3-15）
...
- **排序(基本算法题 要掌握)**
  ...
  - 冒泡排序
- **堆栈、队列、链表**
  - 堆栈(一定要准备)、队列(要准备)、链表(难度较大，来不及可以不准备)
  - Array 具备堆栈和队列的特性.pop push shift unshift
  - 参考链接 https://juejin.im/entry/58759e79128fe1006b48cdfd
- **递归**
  - 一定不能偷懒 60%题用到。终止条件，参数传递
  - 参考链接 https://segmentfault.com/a/1190000009857470
- **波兰式和逆波兰式**
  - 解决一类型题目，扩充知识面，实在没时间不准备
  - 理论 http://www.cnblogs.com/chenying99/p/3675876.html
  - 源码 https://github.com/Tairraos/rpn.js/blob/master/rpn.js
- **面试技巧**
  - 不一定都答对，比别人分高就行
  - 上述基本功准备好
  - 拿到题目先理解。实在理解不了问面试官可不可以给一些提示，这个做法很礼貌也很正常也是权利。得到提示后，大概知道了需要哪种方法。然后先写出思维逻辑、伪代码，然后再分解梳理，能写到哪步就写到哪步。卡住了也可以问面试官，如果在提示下可以答好那么也很棒。
  - 如果一点写不出来，那么展示知识面，概念、原理、在哪儿看过等等。

## |**渲染机制**|

> imooc 《前端跳槽面试必备技巧》（4-1 二面/三面）

- **面试技巧**
  - 知识面要广(一面注重基础知识 二面注重知识面)
  - 理解要深刻(知识体系的深度 如浏览器特性、js 引擎)
  - 内心要诚实(没了解过不要说忘了 问面试官学习资料)
  - 态度要谦虚(不要觉得是故意难为你;也不要膨胀)
  - 回答要灵活(不要和面试官争对错，可以说下来查资料看一下)
  - 要学会赞美(问的很深，或自己没接触，没深入学习过，被问住了很正常。如何快速结束这个话题，说好话，赞美他，说他研究地深，自己研究地没那么深。)
- **渲染机制** 纯理论
  - 什么是 DOCTYPE 及作用
    - DTD？
    - DOCTYPE 是用来声明文档类型和 DTD 规范的？
    - 常见 DOCTYPE 及书写 HTML5 的 DOCTYPE
      - HTML5：\<!DOCTYPE html>
      - HTML4.01 Strict(严格模式)：该 DTD 包含所有 HTML 元素和属性，但不包括展示性的和弃用的元素(如 font)
      - HTML4.01 Transitional(传统模式)：该 DTD 包含所有 HTML 元素和属性，包括展示性的和弃用的元素(如 font)
  - 浏览器渲染过程（再看看？）
    ![浏览器渲染过程](https://upload-images.jianshu.io/upload_images/2310756-ba68b6bd7b6f6a75.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/655/format/webp)
  - 重排 Reflow
  - 重绘 Repaint
  - 布局 Layout(浏览器的，不是 CSS 的)
- **JS 运行机制**
  - JS 引擎以及浏览器和 JS 之间怎么完成交互
  - 看《前端 javascript 面试技巧》，还有找的那篇慕课手记，内容一样
  - 如何理解 js 单线程
    - 同一时刻 js 只能做一件事
  - Event Loop
    - 同步任务->运行栈->空->任务队列放入运行栈  
      异步任务->(到时间后)->任务队列
  - ## 什么是任务队列
  - 异步任务情况
- **页面性能**

  - 你如何保证自己的代码写出来的页面是流畅的。或提升页面性能的方法有哪些
    1. 资源压缩合并，减少 HTTP 请求
    2. 非核心代码异步加载
    - 动态脚本加载: document.createElement('script')
    - defer: script 标签的属性。在 HTML 解析后之后才会执行，如果是多个，按照加载的顺序依次执行。
    - async: script 标签的属性。在加载完之后立即执行，如果是多个，执行顺序和加载顺序无关。
    3. 利用浏览器缓存。缓存相关的 http 协议头。
    - 强缓存:
      - Expires：过期时间。服务器的绝对时间
      - Cache-Control：客户端的时长，秒为单位
    - 协商缓存:
      - Last-Modified：浏览器给头增加一个时间
      - If-Modified-Since：和 Last-Modified 值一样，向服务器询问
      - Etag：文件的哈希值
      - If-None-Match：用 Etag 和服务器的文件匹配
    4. 使用 CDN
    5. 预解析 DNS
    - \<link rel="dns-prefetch" href="//host_name_to_prefetcn.com"> // DNS 预解析，https 协议时浏览器默认关闭
    - \<meta http-equiv="x-dns-prefetch-control" content="on"> // 强制打开浏览器的 DNS 预解析

- **错误监控**
  - 各大公司均有代码质量体系，会通过代码提交时来控制
  - 线上环境的错误收集，错误监控
  - 前端错误的分类
    - 即使运行错误：代码错误
    - 资源加载错误
  - 错误的捕获方式（再看看？）
    ...
    - 资源加载错误
      - object.onerror //img script 标签都有 onerror 事件。
      - performance.getEntries()：获取所有已加载的资源数组
      - Error 事件捕获
    - 跨域的 js 运行错误如何捕获，错误提示什么，怎么处理
      - 会提示错误但没有错误详情
      - 拿到详细信息
        - 客户端：在 script 标签增加 crossorigin 属性
        - 服务端：设置 js 资源响应头 Access-Control-Allow-Origin:\*或域名
  - 上报错误的基本原理
    - 采用 Ajax 通信的方式上报
    - 利用 Image 对象上报（公司都采用这种）
      - `(new Image()).src = 'http://...';`
    - 题外：Image 对象不是 js 内置对象，是 html dom 对象？？？forEach()???

## |**MVVM 框架类**|

> imooc 《前端跳槽面试必备技巧》（4-5）

- 了解 MVVM 框架吗
  - 想好说哪个。话别说太多给后边留余地。谦虚谨慎。
- 谈谈你对 MVVM 的认识
  - 先聊 MVC(model view controller)
  - 再聊 MVVM 定义(Model <-> ViewModel <-> View)
  - 可以再对比 MVVM 和 MVC
- 双向绑定是什么原理，手写
  - view data
  - data->view:Object.defineProperty view->data:input 事件
    - obj.defineProperty 与 reflect.defineProperty 区别
    - obj.defineProperty 手写
- 使用了什么设计模式
  - 观察者设计模式原理
  - 写出伪代码
  - 没有问，也要找时机表现出来
- 生命周期是什么
  - 什么时候可以操作 DOM 了-mounted
- 看过源码吗（看完源码课再看这节）
...
- **面试技巧** 非常重要
  - 准备要充分
    - 项目原理和技术难点
  - 描述要演练
    - 讲怎么做的架构，项目中的哪个难点，重点是什么，展示给面试哪些点怎么描述。掌握时间。
  - 引导找时机
    - 不问也要引导他，找时机说出来。
  - 优势要发挥
  - 回答要灵活
...
> imooc 《前端跳槽面试必备技巧》（第 7 章）

- 九宫格考 CSS 综合实力
  - **警示！！！警示！！！警示！！！**
    1. 不一定每道题都先要想出多种方法，先完成再完美。
    2. 难的解法放在后面，先写出简单常规的方式，不要小看简单的解题思路，思考全面些。不要炫技不成反而题目完成不了。
    3. 有全局观，不要一道题浪费太多时间。
- flat 函数设计
...
> imooc 《ES6 零基础教学 解析彩票项目》

### **let const**

- let const 都是块级作用域
- const 定义时必须声明
- const 定义对象，引用不能修改，内部属性可以修改
...
- **数组解构赋值**
  - 例子
    - `[a,b]=[1,2];` //log(a,b) 1 2
    - `[a,b,...rest]=[1,2,3,4,5,6];` //log(a,b,rest) 1 2 [3,4,5,6]
    - `[a,b,c=3]=[1,2];` //默认值 log(a,b,c) 1 2 3
    - `[a,b,c]=[1,2];` //log(a,b,c) 1 2 undefined
  - 应用场景
    - 变量交换
    ```
    let a=1; let b=2;
    ...
    ```
- **对象解构赋值**
  - `{p,q}={p:42,q:true};` // log(p,q) 42 true
  - `{a=10,b=5}={a:3};` // log(a,b) 3 5
  - 嵌套结构
  ```
  let metaData = {
    title: 'abc',
...
- 构造函数
  - `var regex = new RegExp('xyz','i');` //ES5 创建正则的一种方式
  - `var regex2 = new RegExp(/xyz/i);` //ES5 创建正则的另一种方式
  - `var regex3 = new RegExp(/xyz/ig, 'i');` // ES6 中新增。第二个参数会覆盖前面的 flag。还新增 regex3.flags 属性，输出 i
- 修饰符
  - y sticky 修饰符
  ```
  let s = 'bbb_bb_b';
...
```
[10, 20, 30, 40, 50, 60].find((item) => {
  return item > 30
}); //40 只找第一个，元素值

[10, 20, 30, 40, 50, 60].findIndex((item) => {
  return item > 30
}); //3 只找第一个，下标
```
...
[1,2,NaN].includes(NaN); //true 解决了NaN的问题
```

### **函数扩展**

- 参数默认值。有默认值的参数后面不能再写没有默认值的参数。
...
- rest 参数。把离散的值转化为数组。可传任意数目的参数。rest 参数后不能再有其它参数。
...
test4(1, 2, a); // rest 1   rest 2   rest a
```
...
```
let a1 = Symbol(); //不用new运算符
let a2 = Symbol();
console.log(a1 === a2); // false
let a3 = Symbol.for('a3'); //重新使用一个值
...
- for in, let of 不能取到 symbol 值作为 key 的属性
...
Reflect.ownKeys(obj).forEach(function(item){
  console.log(obj.item); // 可以取到symbol
})
```
...
- **Set**
  ...
  list.add(5);
  list.size; // 1
  list.add(5); // 重复添加不报错，只是不生效

  let arr = [1,2,3,1,'2'];
  let list2 = new Set(arr); // Set{1,2,3,'2'} 去重，且不进行数据类型转换
  ```
  ...
  - for of 循环，以下前三种写法结果都一样

  ```
  for item of list
  for key of list.keys()
  for value of list.values()
  for [key,value] of list.entries() // add add... key value同所以输出两遍
  list.forEach(function(item){console.log(item)}) //同前三种
  ```
...
- **Map**
  ...
  map.set(arr, 456); // 添加元素用set(key, value)
  map.get(arr); // 取值用get
  ```
  ...
  map.delete('a'); // true
  map.clear();
  keys() values() entries() forEach() 均同Set
  ```
...
- **Map Set Array Object 的增删改查**
  ...
  let index = arr.findIndex(item => item.age);
  arr.splice(index, 1);
  ...
  set.forEach(item => item.age ? item.age=18 : ''); //或者直接修改Set中引用的元素值

  arr.splice(0, 1, {'age': 20});
  arr.forEach(item => item.age ? item.age=18 : '');
...
let moniter = new Proxy(obj, {
  ...
  ownKeys(target){
    return Object.keys(target).filter(item => item.key != 'time')
  }
})
...
class Child extends Parent{
  constructor(name = 'child'){
    super(name); // super的参数列表就是父类的参数列表。如果为空会默认使用父类的参数。super放在第一行
    this.type = 'child'; // 子类属性要放在super之后
  }
}
```
...
class Parent{
  ...
  static tell(){ // 这是方法。static关键字。静态方法通过类调用，而不是通过类的实例调用。
    console.log('tell');
  }
}
Parent.type = 'test'; // 静态属性 console.log(Parent.type);
```
...
- **隐式转换**
  ...
  - a === b
    - 类型不同,返回 false
    - 类型相同
      NaN != NaN // NaN 和任何东西比较都不相等，包括自己
      new Object != new Object

- **包装对象**

  - number string boolean 有对应的包装类型

  ```
  var str = "string"; //基本类型
  var strObj = new String("string"); //包装类

  str.length; // 基本类型没有属性和方法，但如果把它当成类使用时，js会自动将基本类型转化为包装类型对象。完成用户操作后该临时包装对象会被销毁，所以赋值完成后再输出结果为undefined
  str.t = 10; // 10
  str.t // undefined
  ```
...
- **类型检测**
  - typeof
    - 适合基本类型和 function 的判断,遇到 null 失效。判断 null 可以通过===。返回一个类型字符串。
    ```
    typeof 100 //number
    typeof true //boolean
    ...
    typeof [1, 2] //object
    typeof NaN //number
    typeof null //object 兼容问题历史原因
    ```
  - instanceof
    - 适合原生对象和自定义对象。
    - `obj instanceof Object` 左边期望是一个对象否则返回 false，右边期望是一个函数对象构造器否则报 typeError
    - 判断左边操作数的对象的原型链上是否有右边这个构造函数的 prototype 属性
    - 不同 window 或 iframe 间的对象类型检测不能使用 instanceof
  - Object.prototype.toString
    - 适合基本类型和内置对象。
    - IE678 下 null 和 undefined 返回[object Object]
    ```
    ...
    ```
  - constructor
    - 任何一个对象都有 constructor 属性，继承自原型的。指向构造这个对象的构造函数。
    - 可以被改写，使用要小心。
  - duck type
    - 如判断数组，可以判断该操作数的长度是否是数字，是否有 join(),push()等函数，通过特征来判断。
  - 编程练习题

## |**表达式和运算符**|

> imooc 《JavaScript 深入浅出》 （第 2 章）
...
- 数组对象的初始化表达式
  ...
  - {x:1, y:2}
- 函数表达式
  - var fe = function(){}; //匿名函数赋值给变量
  - (function(){console.log('hello')})(); //函数用括号括起来然后直接调用
- 属性访问表达式
  - var o = [x:1]; o.x; o['x'];
- 调用表达式
  - func();
...
- 运算符 ?:
- 运算符 ,
  - var val = (1,2,3); //val = 3 逗号运算符
- 运算符 delete
  ```
  var obj = {x:1};
  obj.x; //1
  delete obj.x; //configurable为true时可以delete
  obj.x; //undefined
  ```
- 运算符 in
  ```
...
- 运算符 this
- 运算符 void
  - void() // 不管参数是多少，都返回 undefined
- 运算优先级

## |**语句**|

> imooc 《JavaScript 深入浅出》 （第 3 章）
...
- try catch
  ...
  - 异常捕获的机制。首先执行 try 块中的代码，如果抛出异常会由 catch 从句去捕获并执行。如果没有发生异常 catch 中的代码会被忽略。不管有没有异常都会执行 finally。try 后面必须接着一个 catch 或者 finally，即 catch 和 finally 最多可以省略一个。
  ...
  // 内层没有catch，会跳到最近一层的catch处理，但在跳出之前先要执行finally
  // finally
  // outer oops
  ```
...
- for...in
  ...
  - for in 对象属性受原型链影响，原型链上可枚举属性也会出现
- switch
...
- 严格模式是一种特殊的执行模式，它修复了部分语言上的不足，提供更强的错误检测，并增强安全性。

```
function func(){
  'use strict'; //只function执行严格模式
}

'use strict'; //整个文件执行严格模式
function func(){
...
// a被修改为100。如果不传参，a为undefined。arguments[0]赋值后a仍为undefined。
...
!function(a) {
  ...
  Object.defineProperty(obj,
    'a',{configurable: false});//删除configurable为false的属性
...
- 一般函数调用时(不是对象的方法调用，也不使用 apply/call/bind 等修改 this)this 指向 null,而不是全局对象。若使用 apply/call,当传入 null 或 undefined 时,this 将指向 null 或 undefined,而不是全局对象。
